...
public:
    ...
    int recursion(int pos, int currSum, int product, int isOdd, int k, int n,
        ...
        if (pos == n) {
            return (currSum == k && isOdd != 0 && product <= limit ? product
                                                                   : MIN);
        }
        ...
        int ans =
            recursion(pos + 1, currSum, product, isOdd, k, n, nums, limit);
        if (isOdd == 0) {
        ...
        if (isOdd == 1) {
            ans = max(ans, recursion(pos + 1, currSum + nums[pos],
                                     min(product * nums[pos], limit + 1), 2, k,
                                     n, nums, limit));
        }
        if (isOdd == 2) {
            ans = max(ans, recursion(pos + 1, currSum - nums[pos],
                                     min(product * nums[pos], limit + 1), 1, k,
                                     n, nums, limit));
        }
