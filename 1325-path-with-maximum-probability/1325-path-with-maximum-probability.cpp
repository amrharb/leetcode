...
class Solution {
public:
    void dfs(vector<vector<pair<int, double>>>& graph, vector<bool>& vis,
             int src, int dst, double curres, double& res, double& mn) {
        if (curres <= mn)
            return;
        if (src == dst) {
            res = max(res, curres);
    ...
    double maxProbability(int n, vector<vector<int>>& e,
                          vector<double>& succProb, int src, int dst) {
